...
bool FilterGeneric::setB(float32 *b, size_t length) {
  if (length != m_bSize)
    return false;

  std::memcpy(m_b, b, sizeof(float32) * length);

  return true;
}

bool FilterGeneric::setA(float32 *a, size_t length) {
  if (length != m_aSize)
    return false;

  std::memcpy(m_a, a, sizeof(float32) * length);
